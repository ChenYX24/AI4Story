...
def interact(req: InteractRequest, background_tasks: BackgroundTasks) -> InteractResponse:
    if not req.ops:
    ...
    if not LLM_API_KEY:
        raise HTTPException(status_code=424, detail="服务器未配置 LLM_API_KEY（叙事生成需要）")
    try:
        payload = generate_dynamic_node(req, background_tasks)
